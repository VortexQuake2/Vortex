...
static constexpr int SET_HARD_MODE_MONSTERS[] = {
    DS_PARASITE, DS_BRAIN, DS_MEDIC, DS_TANK, DS_TANK_N64, DS_MUTANT, DS_GLADIATOR, DS_BERSERK, DS_INFANTRY, DS_ENFORCER, DS_HOVER,
    DS_REDMUTANT, DS_RUNNERTANK, DS_GUNCMDR, DS_DAEDALUS, DS_BOSS2_SMALL, DS_GLADB, DS_GLADC, DS_STALKER, DS_GEKK, DS_ARACHNID, DS_BITCH_HEAT
};
...
static constexpr int SET_MELEE_MONSTERS[] = {
    DS_PARASITE, DS_BRAIN, DS_MUTANT, DS_BERSERK, DS_REDMUTANT, DS_STALKER, DS_GEKK, DS_ARACHNID,
};
constexpr int SET_MELEE_MONSTERS_COUNT = sizeof(SET_MELEE_MONSTERS) / sizeof(int);

// gladiator, berserker
static constexpr int SET_RAGEQUIT_MONSTERS[] = {
    DS_GLADIATOR, DS_BERSERK,
};
constexpr int SET_RAGEQUIT_MONSTERS_COUNT = sizeof(SET_RAGEQUIT_MONSTERS) / sizeof(int);

// tank, mutant, berserker, shambler!
static constexpr int SET_TANKY_MONSTERS[] = {
    DS_TANK, DS_TANK_N64, DS_MUTANT, DS_BERSERK, DS_SHAMBLER, DS_REDMUTANT, DS_RUNNERTANK, DS_GUNCMDR, DS_GLADB, DS_GLADC, DS_GEKK, DS_ARACHNID, DS_ENFORCER
};
constexpr int SET_TANKY_MONSTERS_COUNT = sizeof(SET_TANKY_MONSTERS) / sizeof(int);

// parasites!
static constexpr int SET_PARASITE_MONSTERS[] = {
    DS_PARASITE
};
...
void vrx_inv_init_post_entities(void) {
    ...
    gi.dprintf("Invasion: %d start navis, %d total. %d player spawns, %d monster spawns.\n", invasion_start_navicount,
               invasion_navicount, invasion_spawncount, invasion_monsterspawns);
}
...
edict_t *vrx_inv_closest_start_navi(edict_t *self) {
    vec3_t eorg;
...
edict_t *drone_findnavi(edict_t *self) {
    ...
    if (invasion_start_navicount > 0) {
        ...
        // if we are told to simply use the closest navi, do so
        if (self->monsterinfo.aiflags & AI_SNAP_TO_NAVI) {
...
void vrx_inv_award_players(void) {
    ...
    for (i = 0; i < game.maxclients; i++) {
        ...
        if (invasion->value == 2)
            points = 2.0f / 5.0f * player->client->resp.score * ((float) num_spawns / invasion_max_playerspawns) + 300 *
                     sqrt(next_invasion_wave_level);
        else
        ...
        if (invasion->value < 2)
            ...
            credits = 2.0f / 5.0f * INVASION_BONUS_CREDITS * ((float) num_spawns / invasion_max_playerspawns) + 300 *
                      sqrt(next_invasion_wave_level);
...
void vrx_inv_check_spawn_camp(edict_t *monsterSpawn) {
    ...
    while ((e = findradius(e, monsterSpawn->s.origin, 256)) != NULL) {
        ...
        if (e->svflags & SVF_MONSTER && e->health > 0 && e->activator && e->activator->inuse && !e->activator->client &&
            visible(monsterSpawn, e)) {
...
void info_player_invasion_think(edict_t *self) {
    if (level.time > self->lasthurt + 1.0)
        M_Regenerate(self, PLAYERSPAWN_REGEN_FRAMES, PLAYERSPAWN_REGEN_DELAY, 1.0, true, false, false,
                     &self->monsterinfo.regen_delay1);
