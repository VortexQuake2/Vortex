...
void GetSuccessorNodes(node_t *StartNode, int NodeNumS, int NodeNumD) {
    ...
    Successor->h = h = distance(pathnode[NodeNumS], pathnode[NodeNumD]);
...
int FindPath(int searchType, vec3_t start, vec3_t destination) {
    ...
    StartNode->h = h = distance(start, destination);
...
void DrawPath1(void) {
    ...
    for (i = 0; i < numpts; i++) {
        ...
        G_Spawn_Splash(TE_LASER_SPARKS, 4, 0xdcdddedf, pathnode[j], vec3_origin, pathnode[j]);
#else
        gire.Draw_Arrow(pathnode[i], pathnode[j], 1, &rgba_blue, &rgba_blue, FRAMETIME, true);
#endif
...
void DrawChildLinks(edict_t *ent) {
    ...
    if (!(level.framenum % 20)) {
        auto s = va("Node %d has %d child links @ %.0f.\n",
