...
#include "../combat/common/v_damagelist.h"
...
void boss_update(edict_t *ent, usercmd_t *ucmd) {
    ...
    if (!(ent->client->buttons & BUTTON_ATTACK)) {
        if (forwardspeed > 0) {
            M_walkmove(boss, boss->s.angles[YAW], norm_fspeed);
        ...
        if (sidespeed > 0) {
            M_walkmove(boss, angles[YAW], norm_sspeed);
            frames = FRAMES_RUN_FORWARD;
        } else if (sidespeed < 0) {
            M_walkmove(boss, angles[YAW], norm_sspeed);
            frames = FRAMES_RUN_FORWARD;
    ...
    if (boss->groundentity) {
        ...
        if (cmd_jumping(ucmd) && !ent->client->jump) {
        ...
        if (CanDoubleJump(boss, ucmd)) {
            boss->velocity[2] += scale_fps(350);
            boss->v_flags |= SFLG_DOUBLEJUMP;
        }

        // steer in the direction we are trying to move
        boss->velocity[0] += scale_fps(0.1 * ((forwardspeed * forward[0]) + (sidespeed * right[0])));
        boss->velocity[1] += scale_fps(0.1 * ((forwardspeed * forward[1]) + (sidespeed * right[1])));

        if (!cmd_standing(ucmd) && (boss->waterlevel > 1)) {
            if (cmd_jumping(ucmd)) {
                if (boss->waterlevel == 2)
                    boss->velocity[2] = scale_fps(200);
                else if (boss->velocity[2] < scale_fps(50))
                    boss->velocity[2] = scale_fps(50);
            } else {
                if (boss->velocity[2] > scale_fps(-50))
                    boss->velocity[2] = scale_fps(-50);
            }
        }
