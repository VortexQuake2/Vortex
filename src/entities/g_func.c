...
//		(int)ent->s.origin[2], (int)ent->moveinfo.end_origin[2], (int)ent->moveinfo.remaining_distance);
	...
	VectorScale (ent->moveinfo.dir, ent->moveinfo.current_speed * sv_fps->value, ent->velocity);
	ent->nextthink = level.time + FRAMETIME;
	ent->think = Think_AccelMove;
}
...
void plat2_hit_top (edict_t *ent)
{
	...
	if (ent->plat2flags & PLAT2_CALLED)
	{
		...
		if (!(ent->spawnflags & SPAWNFLAGS_PLAT2_TOGGLE))
		{
			...
			ent->nextthink = level.time + ent->wait * 2.5;
		}
		if (deathmatch->integer)
			ent->last_move_time = level.time - ent->wait * 0.5;
		else
			ent->last_move_time = level.time - ent->wait;
	}
...
void plat2_operate (edict_t *ent, edict_t *other)
{
	...
	if ((ent->last_move_time + ent->wait) > level.time)
		return;
...
void SP_func_plat2 (edict_t *ent)
{
	...
	ent->blocked = plat2_blocked; // Vortex: blocked lives on the edict, not moveinfo
