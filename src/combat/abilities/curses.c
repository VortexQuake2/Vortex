...
qboolean curse_add(edict_t *target, edict_t *caster, int type, int curse_level, float duration)
{
    ...
    talentLevel = vrx_get_talent_level(caster, TALENT_EVIL_CURSE);
    if(talentLevel > 0)
    {
        //Curses only
...
void Bleed (edict_t *curse)
{
	...
	// 50-99% health taken over duration of curse at level 10
	...
	T_Damage(curse->enemy, caster, caster, vec3_origin, vec3_origin, 
		vec3_origin, take, 0, DAMAGE_NO_ABILITIES, MOD_LIFE_DRAIN);//FIXME: MoD

	// bleed (damage) every 3-5 frames (0.3 - 0.5 second)
	curse->wait = level.time + (GetRandom(3, 5) * FRAMETIME);
}
...
{  
	...
	// scan for targets  
	while ((target = findclosestradius_targets(target, ent, radius)) != NULL)  
	{   
		if (target == ent)    
			continue;
