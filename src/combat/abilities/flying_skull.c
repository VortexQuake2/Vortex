        ...
        */
        ...
        while (1) // if we want to save cpu cycles, we could limit this to x vertical steps
        {
            ...
            if (tr.fraction < 1) {
                ...
                return self->s.origin[2];
                // obstruction below, can't proceed any further (FIXME: returned value should be outside valid Z range, or just return current Z height)
            }
...
void skull_think(edict_t *self) {
    ...
    if (G_ValidTarget(self, self->enemy, false, true) // is the target still valid?
        && entdist(self->activator, self->enemy) <= SKULL_MAX_RANGE)
    // make sure the target isn't too far from activator (don't wander)
    {
        if (!visible(self, self->enemy)) {
...
void SpawnSkull(edict_t *ent) {
    ...
    skull->health = SKULL_INITIAL_HEALTH + SKULL_ADDON_HEALTH * ent->myskills.abilities[HELLSPAWN].current_level;
    //ent->myskills.level;
    skull->dmg = SKULL_INITIAL_DAMAGE + SKULL_ADDON_DAMAGE * ent->myskills.abilities[HELLSPAWN].current_level;
    //*ent->myskills.level;
