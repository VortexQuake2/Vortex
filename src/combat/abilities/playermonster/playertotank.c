...
void PM_RegenAmmo(edict_t *monster, int regen_frames, int delay) {
    ...
    if (monster->owner && monster->owner->inuse
        ...
        if (mode == 0)
            ...
            regen_bullets = false;
        else if (mode == 3) // az: nov/2022 this was missing. FOR HOW LONG.
            regen_cells = false;
    }
...
void p_tank_spawn(edict_t *ent, int cost) {
    ...
    tank->monsterinfo.jumpdn =
            P_TANK_INITIAL_ROCKETS + P_TANK_ADDON_ROCKETS * tank->monsterinfo.level; // max rocket ammo
    tank->monsterinfo.lefty = P_TANK_SPAWN_BULLETS; // starting bullets
    tank->monsterinfo.search_frames =
            P_TANK_INITIAL_BULLETS + P_TANK_ADDON_BULLETS * tank->monsterinfo.level; // max bullets
