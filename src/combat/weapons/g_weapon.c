...
=================
*/
static void fire_lead(edict_t *self, vec3_t start, vec3_t aimdir, float damage, int kick, int te_impact, int hspread,
                      int vspread, int mod) {
    trace_t tr;
    vec3_t dir;
    vec3_t forward, right, up;
    ...
    if (!((tr.surface) && (tr.surface->flags & SURF_SKY))) {
        if (tr.fraction < 1.0) {
            ...
            if (self->client) {
                ...
                /*if (mod == MOD_SHOTGUN)
                    rangeLevel = self->client->resp.pstats.weapons[WEAPON_SHOTGUN].mods[1].current_level;
                else*/
                if (mod == MOD_SSHOTGUN)
                    rangeLevel = self->client->resp.pstats.weapons[WEAPON_SUPERSHOTGUN].mods[1].current_level;
...
=================
*/
void fire_bullet(edict_t *self, vec3_t start, vec3_t aimdir, float damage, int kick, int hspread, int vspread,
                 int mod) {
...
void WeaponStun(edict_t *attacker, edict_t *target, int mod) {
    ...
    if (mod == MOD_BLASTER) {
        ...
        lvl = attacker->client->resp.pstats.weapons[WEAPON_BLASTER].mods[1].current_level;
        value += 0.033 * lvl; // 25% at level 10
    } else if (mod == MOD_HYPERBLASTER) {
        duration = 0.1;
        lvl = attacker->client->resp.pstats.weapons[WEAPON_HYPERBLASTER].mods[1].current_level;
...
void Grenade_Explode(edict_t *ent) {
    ...
    if (ent->enemy) {
        ...
        T_Damage(ent->enemy, ent, ent->owner, dir, ent->s.origin, vec3_origin, (int) points, (int) points,
                 DAMAGE_RADIUS, mod);
    }
    ...
    if (Q_stricmp(ent->classname, "grenade") == 0) {
        //	ent->dmg = GRENADELAUNCHER_INITIAL_DAMAGE + GRENADELAUNCHER_ADDON_DAMAGE * ent->client->resp.pstats.weapons[WEAPON_GRENADELAUNCHER].mods[0].current_level) / 2;
...
void fire_grenade(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius,
    ...
    //K03 Begin
    if (!self->client ||
        self->client->resp.pstats.weapons[WEAPON_GRENADELAUNCHER].mods[3].current_level < 1)
...
edict_t *fire_grenade2(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer,
                       float damage_radius, int radius_damage, qboolean held) {
    edict_t *grenade;
    vec3_t dir;
    vec3_t forward, right, up;
    vec3_t adjusted_aim;
    ...
    //K03 Begin
    if (!self->client ||
        self->client->resp.pstats.weapons[WEAPON_HANDGRENADE].mods[3].current_level < 1)
        grenade->s.effects |= EF_GRENADE;
    //K03 End
    VectorClear(grenade->mins);
    VectorClear(grenade->maxs);
...
void fire_rocket(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius,
    ...
    //K03 Begin
    if ((self->client &&
         self->client->resp.pstats.weapons[WEAPON_ROCKETLAUNCHER].mods[3].current_level < 1) || !self->client)
...
void fire_rail(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick) {
    ...
    while (ignore && iters++ < 500) {
        ...
        if (tr.contents & (CONTENTS_SLIME | CONTENTS_LAVA)) {
            ...
            if ((tr.ent != self) && (tr.ent->takedamage)) {
                ...
                if (T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, DAMAGE_PIERCING,
                             mod)) {
                    if (self->client &&
                        self->client->resp.pstats.weapons[WEAPON_RAILGUN].mods[2].current_level > 0)
                        burn_person(tr.ent, self,
                                    (int) (RAILGUN_ADDON_HEATDAMAGE * self->client->resp.pstats.weapons[WEAPON_RAILGUN].
                                           mods[2].current_level));
                }
    ...
    if (!self->client || particles->value || (!self->client->weapon_mode
                                              && (self->client->resp.pstats.weapons[WEAPON_RAILGUN].mods[3].
                                                  current_level < 1))) {
        // we can get here with null self client
        if (self->client &&
            self->client->resp.pstats.weapons[WEAPON_RAILGUN].mods[0].current_level >= 5) {
            gi.WriteByte(svc_temp_entity);
...
void fire_20mm(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, float range) {
    ...
    while (ignore && itercount++ < 500) {
        ...
        if (tr.contents & (CONTENTS_SLIME | CONTENTS_LAVA)) {
            ...
            if ((tr.ent != self) && (tr.ent->takedamage)) {
                T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, DAMAGE_PIERCING,
                         MOD_CANNON);
                //if (self->client->resp.pstats.weapons[WEAPON_RAILGUN].mods[2].current_level > 0)//K03
                //burn_person(tr.ent, self, (int)(RAILGUN_ADDON_HEATDAMAGE * self->client->resp.pstats.weapons[WEAPON_RAILGUN].mods[2].current_level));
            }
        }
        ...
        if (ducked == false) {
            ...
            auto val = self->client ? self->client->resp.pstats.weapons[WEAPON_20MM].mods[2].current_level : 0;
            self->velocity[0] -= forward[0] * (500 - (25 * val));
            self->velocity[1] -= forward[1] * (500 - (25 * val));
        }
        //GHz End
    }
}
...
void bfg_think(edict_t *self) {
    ...
    while ((target = findradius(target, self->s.origin, range)) != NULL) {
        ...
        VectorScale(v, self->owner->client->resp.pstats.weapons[WEAPON_BFG10K].mods[4].current_level*5, v);
...
void fire_bfg(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius) {
    ...
    if (self->client) {
        ...
        bfg->random = BFG10K_INITIAL_DURATION + BFG10K_ADDON_DURATION * self->client->resp.pstats.weapons[WEAPON_BFG10K]
                      .mods[1].current_level;
        // slide upgrade
        bfg->radius_dmg = self->client->resp.pstats.weapons[WEAPON_BFG10K].mods[3].current_level;
    } else {
