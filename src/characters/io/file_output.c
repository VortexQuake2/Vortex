...
void vrx_save_character(edict_t *ent, qboolean unlock)
{
	...
	for (i=0; i<game.num_items; i++, item++)
		ent->client->resp.pstats.inventory[ITEM_INDEX(item)] = temp[ITEM_INDEX(item)];
	...
	Q_strncpy (oldPassword, vrx_encrypt_string(ent->client->resp.pstats.password, true), sizeof(oldPassword) - 1);

	if(Q_stricmp(oldPassword, newPassword) != 0)
		vrx_write_to_logfile(ent, va("Changed password from '%s' to '%s'.\n", oldPassword, newPassword));

	//Encrypt the password.
	Q_strncpy (ent->client->resp.pstats.password, vrx_encrypt_string(newPassword, false), sizeof(ent->client->resp.pstats.password) - 1);

	//3.0 save file
	if (!vrx_commit_character(ent, unlock))
		gi.dprintf("ERROR: vrx_save_character() unable to save player: %s", ent->client->resp.pstats.player_name);
}